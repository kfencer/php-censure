		...
		$charset = 'UTF-8')
	{
		...
		if ($re_badwords === null)
		{
			...
			$badwords = array(
				#Слово на букву Х
				'(?<=\PL) %RE_PRETEXT%?
                      [hхx][уyu]_?[ийiеeёяюju]     #хуй, хуя, хую, хуем, хуёвый, охуительный
                      #исключения:
				...
				'(?<=\PL) %RE_PRETEXT%?
                      [eеё]_?
							#исключения
							(?<!н[eе][её]_|[eеё]_|н[eеё]_|т_е_)    #неё, т.е. большие
				...
				'(?<=\PL) %RE_PRETEXT%
                      (?<= \pL\pL|\pL_\pL_)
                      [eеё][бb6]    #долбоёб, дураёб, изъёб, заёб, заебай, разъебай, мудоёбы
            ',
